...
def explain_fls_decision_plot(text):
    import matplotlib
    matplotlib.use("Agg")  # Ensure compatibility with Gradio
    import matplotlib.pyplot as plt
    import io, base64

    shap_values = fls_explainer([text])

    # Select predicted class
    probs = fls_predict([text])[0]
    pred_class = np.argmax(probs)   # 0 = Forward-Looking, 1 = Not Forward-Looking

    # Create waterfall plot for that class
    ...
    """
    for label, score in zip(fls_labels, probs):
        prob_table += f"<tr><td style='text-align:right;'>{label}</td><td style='text-align:right;'>{score:.2f}</td></tr>"
    prob_table += "</table></div>"

    return f"""
    <h3>FLS SHAP Waterfall Plot (Class: {fls_labels[pred_class]})</h3>
    <div style='text-align:center;'>
