import os
import gradio as gr
from transformers import pipeline, AutoTokenizer, AutoModelForTokenClassification
import spacy
import io
from PIL import Image

# Install required packages
os.system("pip install gradio==3.0.18")
os.system("pip install librosa")
os.system("pip install soundfile")
os.system("pip install shap")

import shap
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

# Load spaCy model
nlp = spacy.load("en_core_web_lg")  # Use large model for better accuracy
nlp.add_pipe('sentencizer')

def split_in_sentences(text):
    doc = nlp(text)
    return [str(sent).strip() for sent in doc.sents]

def make_spans(text, results):
    results_list = [results[i]['label'] for i in range(len(results))]
    facts_spans = list(zip(split_in_sentences(text), results_list))
    return facts_spans

# Speech Recognition Pipeline
asr = pipeline(
    "automatic-speech-recognition",
    model="openai/whisper-large-v3",
    chunk_length_s=30,
    stride_length_s=(5, 5),
    device=0
)

def speech_to_text(audio_path):
    if audio_path is None:
        return "Please upload an audio file first."
    try:
        result = asr(audio_path, return_timestamps=True)
        full_text = " ".join([chunk['text'] for chunk in result['chunks']])
        return full_text
    except Exception as e:
        return f"Error processing audio: {str(e)}"

# Summarization Pipeline
summarizer = pipeline("summarization", model="knkarthick/MEETING_SUMMARY")
def summarize_text(text):
    resp = summarizer(text)
    stext = resp[0]['summary_text']
    return stext

# Fiscal Tone Analysis Pipeline
fin_model = pipeline("sentiment-analysis", 
                    model='yiyanghkust/finbert-tone', 
                    tokenizer='yiyanghkust/finbert-tone')

def text_to_sentiment(text):
    sentiment = fin_model(text)[0]["label"]
    return sentiment

def fin_ner(text):
    doc = nlp(text)
    entities = []
    for ent in doc.ents:
        if ent.label_ == "ORG" and len(ent.text) > 2:
            entities.append((ent.start_char, ent.end_char, "COMPANY"))
        elif ent.label_ in ("GPE", "LOC"):
            entities.append((ent.start_char, ent.end_char, "LOCATION"))
    output = []
    last_end = 0
    for start, end, label in sorted(entities, key=lambda x: x[0]):
        if start > last_end:
            output.append((text[last_end:start], None))
        output.append((text[start:end], label))
        last_end = end
    if last_end < len(text):
        output.append((text[last_end:], None))
    return output

def fin_ext(text):
    results = fin_model(split_in_sentences(text))
    return make_spans(text, results)

def fls(text):
    fls_model = pipeline("text-classification", 
                        model="yiyanghkust/finbert-fls", 
                        tokenizer="yiyanghkust/finbert-fls")
    results = fls_model(split_in_sentences(text))
    return make_spans(text, results)

# SHAP integration — Only global explainer
shap_pipeline = shap.models.TransformersPipeline(fin_model)
explainer = shap.Explainer(shap_pipeline, fin_model.tokenizer)

def shap_summary_plot(text, truncate):
    try:
        # Proper token truncation
        tokens = fin_model.tokenizer(text, truncation=True, max_length=400, return_tensors="pt")
        num_tokens = tokens.input_ids.shape[1]
        # If truncation is ON, re-encode text to num_tokens < 400
        if truncate and num_tokens > 400:
            encoded = fin_model.tokenizer(text, truncation=True, max_length=400)
            text = fin_model.tokenizer.decode(encoded["input_ids"], skip_special_tokens=True)
        # SHAP expects a list of texts
        shap_values = explainer([text])
        tokens = shap_values.data[0].tokens
        scores = shap_values.values[0]
        fig, ax = plt.subplots(figsize=(min(16, max(6, len(tokens)*0.4)), 2))
        ax.bar(tokens, scores, color='skyblue')
        ax.set_xticks(range(len(tokens)))
        ax.set_xticklabels(tokens, rotation=45, ha='right', fontsize=8)
        ax.set_title("Token-Level SHAP Importance")
        plt.tight_layout()
        buf = io.BytesIO()
        plt.savefig(buf, format='png')
        buf.seek(0)
        plt.close(fig)
        return Image.open(buf), f"Token count: {num_tokens}"
    except Exception as e:
        print(f"SHAP error: {e}")
        return None, f"Error: {str(e)}"

custom_css = """
<style>
body, html {
    height: 100%;
    margin: 0;
    font-family: Arial, Helvetica, sans-serif;
    background-color: #808000;
    color: #000000;
}
.gradio-container {
    background-color: rgba(255, 255, 255, 0.8);
    padding: 30px;
    border-radius: 15px;
    max-width: 900px;
    margin: auto;
    box-shadow: 0 0 15px rgba(0,0,0,0.2);
}
#logo {
    position: absolute;
    top: 20px;
    right: 20px;
    width: 120px;
    height: auto;
    z-index: 1000;
}
button {
    background-color: #009a4d;
    border: none;
    color: black;
    padding: 12px 20px;
    text-align: center;
    font-size: 16px;
    border-radius: 10px;
    cursor: pointer;
    transition: background-color 0.3s ease;
}
button:hover {
    background-color: #8fbc8f;
}
.tab-nav button[aria-selected="true"] {
    background-color: #8fbc8f !important;
    color: black !important;
    font-weight: bold;
}
#logo {
    position: absolute;
    top: 5px;
    right: 20px;
    width: 80px;
    height: auto;
    z-index: 1000;
}
h1, h2, h3, h4 {
    color: #000000;
}
input, textarea {
    border-radius: 8px;
    border: 1px solid #ccc;
    padding: 10px;
    font-size: 16px;
    width: 100%;
    box-sizing: border-box;
    color: #000000;
    background-color: #000000;
}
</style>
"""

demo = gr.Blocks()
with demo:
    gr.HTML(custom_css)
    gr.Image("https://lh3.googleusercontent.com/pw/AP1GczMzUegdq6DGcyV69iCQ9SqCFfTiS6t8MwMxQNdJP2pwtXEX4KLpMJhIGjVgdoHs9vezs8eA-MWs7GBZsRqzrIVdH0fIrcMHiWiKl3L3O0yt7mShvQ=w2400", elem_id="logo")
    gr.Markdown("## Financial Analyst AI")
    gr.Markdown("This project applies AI trained to analyze earning calls and financial documents.")
    with gr.Tabs():
        with gr.TabItem("Speech Recognition"):
            audio_file = gr.Audio(source="upload", type="filepath", label="Upload Audio (WAV, MP3)")
            text = gr.Textbox(label="Transcribed Text", lines=5, placeholder="Speech transcript output goes here...")
            b1 = gr.Button("Recognize Speech")
            b1.click(speech_to_text, inputs=audio_file, outputs=text)
        with gr.TabItem("Summarization & Tone"):
            text_input = gr.Textbox(label="Input Text", lines=8, placeholder="Paste or type your text here...")
            summary_output = gr.Textbox(label="Summary", lines=4)
            b2 = gr.Button("Summarize")
            b3 = gr.Button("Classify Financial Tone")
            tone_label = gr.Label()
            b2.click(summarize_text, inputs=text_input, outputs=summary_output)
            b3.click(text_to_sentiment, inputs=summary_output, outputs=tone_label)
        with gr.TabItem("In-depth Analysis"):
            fin_spans = gr.HighlightedText(label="Financial Tone Analysis")
            fls_spans = gr.HighlightedText(label="Forward Looking Statements")
            entities_spans = gr.HighlightedText(label="Named Entities", 
                                                color_map={"COMPANY": "#7CFC00", "LOCATION": "#00BFFF"})
            shap_img = gr.Image(label="SHAP Feature Importance")
            token_count = gr.Textbox(label="Token Info", interactive=False)
            truncate_toggle = gr.Checkbox(label="Truncate input if too long", value=True)
            analyze_button = gr.Button("Analyze")
            analyze_button.click(fin_ext, inputs=text_input, outputs=fin_spans)
            analyze_button.click(fls, inputs=text_input, outputs=fls_spans)
            analyze_button.click(fin_ner, inputs=text_input, outputs=entities_spans)
            analyze_button.click(shap_summary_plot,
                                inputs=[text_input, truncate_toggle],
                                outputs=[shap_img, token_count])

if __name__ == "__main__":
    demo.launch(share=True)
