...
def eval_asr(data_csv: str, out_dir: str, device: int, save_raw: bool, chunk_len=30, stride=5):
    ...
    os.makedirs(out_dir, exist_ok=True)
    df = pd.read_csv(data_csv)
    paths = df["audio_path"].astype(str).tolist()
    refs = df["reference_text"].astype(str).tolist()

    asr = pipeline("automatic-speech-recognition", model="openai/whisper-large-v3",
                   chunk_length_s=chunk_len, stride_length_s=(stride, stride), device=device, return_timestamps=True)

    hyps = []
    raw = []
    for p in tqdm(paths, desc="Whisper transcribe"):
    ...
    for r, h, p in zip(refs, hyps, paths):
        per_file.append({"audio_path": p, "wer": wer(r, h), "cer": cer(r, h)})

    pd.DataFrame({"audio_path":paths, "reference":refs, "hypothesis":hyps}).to_csv(os.path.join(out_dir, "predictions.csv"), index=False)
